...
function imageUpdate(){
	...
	if(is_weixn(true)){//微信环境
      ...
      $(".pub-btn").on("click", function () {
  		                    ...
  		                    success: function (res) {
  		                        ...
  		                        $.ajax({
  		                            ...
  		                            success: function (result) {
  		                                if (result.status == 1) {
  		                                    $(".publish-btn").append("<div class='pub-img flex-y' type_id='pic' href=" + result.data +" id="+ i + " style='margin-top:1rem'><div class='del-img' id=" + i + ">删除</div><img src=" + result.data + " id=" + i + "></div>");
        ...
        var source='publish';
        $('.addWrap').find('.pub-btn').remove();
        $('<span class="pub-btn spec">添加图片<input type="file" capture="camera" accept="image/*" multiple="multiple" id="cameraInput" name="cameraInput" /></span>').appendTo($('.addWrap'));
		      ...
		      success: function (data, textStatus) {
		        ...
		        if (data.status == 1) {
		            if(source=="myinfo"){
		                ...
		                $(".publish-btn").append("<a class='pub-img swipebox flex-y' type_id='pic' id="+ index + " href='"+data.data+"' style='margin-top:1rem;'><div class='del-img' id=" + index + ">删除</div><img src=" + data.data + " id=" + index + "></a>");
		            }else{
