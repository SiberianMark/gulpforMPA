    ...
    jsonAjax(API.API_LIST.PAY_CHECKORDER, {
        ...
        if (data.status == 1) {
            ...
            setTimeout(function () {
                ...
                if(!is_pub_acct){
                     $.cookie('isPay'+go_param.id,1,{expires: 30,path:'/'});
...
function checkPwd(){
    ...
    var html='<div class="mui-popup">';
            ...
            html+='<div class="mui-popup-bott">';
                ...
                html+='    </div>';
                html+='</div>';
                html+='<div style="opacity:0">下一步</div>';
                html+='</div>';
            html+='</div>';
        html+='</div>';
...
function setPwdMsg(){
    ...
    dialog.rightBtn=function(){ //right btn click
        ...
        obj.source=I('source','');
        go.plant_source=I('plant_source',0);//是否切换平台文章id
        go.id=I('id',0);//玩家文章id
        go.rid=I('rid',0);//平台文章id
        go.cashier=obj;
        go.source=3;
        go.beforePage=I('beforePage','note');
