...
function getOpenid() {
    if(is_weixn(true)){
        ...
        if (typeof(openid) == "undefined" || openid == '') {
            ...
            var wxappid = I('wxappid', '');
            if (typeof(wxappid) == "undefined" || wxappid == '') {
                wxappid = $.cookie('wxappid');
                if (typeof(wxappid) == "undefined") {
                    wxappid = '';
                }
            }
            event_link(API.SERVER_URL + API.APP_LICATION + API.API_LIST.LOGIN_WEChAT + '?url=' + encodeURIComponent(pageUrl) + '&wxappid=' + wxappid);
            return false;
        }
        return openid;
    }else{
    ...
    jsonAjax(sdk, {'url': encodeURIComponent(location.href.split('#')[0])}, function (data) {
        if (data.status == 1) {
            ...
            wx.ready(function () {
                ...
                try {
                    onWxReady();//微信sdk配置完毕回调函数，各页面根据实际情况实现该函数。
                    /*on wx sdk config ready ,if this needs share*/
                    //使用方法举例：
                    // function onWxReady(){
                    //     to do
                    //     call wx share,show wx share menu
...
function loadWxObject() {
    ...
    //console.log('call loadWxObject',isShare,flag);
    if (is_weixn(true)) {
        if(flag){//初始化JS-SDK对象
            configWxSDK();
        }else if(isShare){//已初始化完成，设置分享信息
            if(WX_READY_STATE!=2){
                if(WX_READY_STATE==0){configWxSDK();}
                WX_INTERVAL=setInterval(function(){
                    if (isonlyfriend) {//只分享到朋友圈
                        onWxShare(true,show);
                    } else {
                        onWxShare(false,show);
                    }
                    if(WX_READY_STATE==2) {clearInterval(WX_INTERVAL);}
                },500);
            }else{
...
//开启分享菜单
function onWxShare() {
    var isonlyfriend = arguments[0] ? arguments[0] : false;//产品需求，已禁用指分享到朋友圈的功能。
    var show = arguments[1] ? arguments[1] : false;//参数为true，弹出app的分享框
    var cabk = arguments[2] ? arguments[2] : null;//分享后回调函数
    /*if (isonlyfriend) {
    ...
    if(window.location.href.indexOf('web/carsvc-landrover')>0||window.location.href.indexOf('web/pub-acct')>0){
        list.splice(2,3);//专属公众号去掉分享qq，复制链接功能
    }
    if (is_weixn(true)&&WX_READY_STATE==2) {
        wx.showMenuItems({
            menuList: list // 要显示的菜单项，所有menu项见附录3
        });
        wxShare(WxShare,cabk);//设置微信分享信息
    }else {
        if (is_smdd()) {
            try {
                var jsonObj=null;
 ...
 * 微信分享
 */
function wxShare(data,cabk) {
    var linkObj=null;
    try{
        linkObj=JSON.parse(data.link);
    }catch(e){linkObj=null;}
    //alert(data.linkUrl?data.linkUrl:linkObj.wechat);
    if (is_weixn(true)) {
        wx.onMenuShareQQ({
        ...
        wx.onMenuShareTimeline({
            title: data.title, // 分享标题
            link: data.linkUrl?data.linkUrl:linkObj.wechat, // 分享链接
            imgUrl: data.imgUrl, // 分享图标
            success: function () {
                if(cabk){ cabk('tl');}
                $('.modal').removeClass('fade');
 ...
 *除法函数，用来得到精确的除法结果
 *返回值：arg1除以arg2的精确结果
 * num参数为保留几位小数---默认保留两位小数---蒋定辉添加
 */
function mathDiv(arg1, arg2,num) {
    var t1 = 0, t2 = 0, r1, r2,n=num?num:2;
    if((''+arg1).indexOf('.')>-1){
...
function redirect(url) {
    // if(url.indexOf('/web/interact/')>0&&url.indexOf('prize-record.html')==-1){
    //     url='/web/lottery/notice.html';
    // }
