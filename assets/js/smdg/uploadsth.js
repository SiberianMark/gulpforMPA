...
function imageUpdate() {
  ...
  var eventType = arguments[1] ? arguments[1] : 'click';
  var selfs = arguments[2] ? arguments[2] : '';
  var len = 0;
  var i = 0; //总共图片张数
  ...
  //删除图片按钮
  $(".publish-btn").on(eventType, ".del-img", function(e) {
    e.stopPropagation();
    e.preventDefault();
    var id = $(this).attr("id");
  ...
  if (is_weixn(true)) { //微信环境
    //微信预览图片的接口.
    $(".publish-btn").on(eventType, ".pub-img img", function(event) {
      //预览图片接口
      var imgArray = [];
              ...
              success: function(res) {
                ...
                $.ajax({
                  ...
                  success: function(result) {
                    if (result.status == 1) {
                      if (selfs != '') {
                        $(".pub-btn").parents('.publish-btn').prev('img').attr('src', result.data);
                        upload_vote(result.data);
                      } else {
    ...
    function postImage(index, base64, source, cbk, end) {
      ...
      $.ajax({
        ...
        success: function(data, textStatus) {
          ...
          if (data.status == 1) {
            if (source == "myinfo") {
              jsonAjax(API.API_LIST.PACKAGE_UPDATEMYINFO, {
                'userid': getUserid(),
                type: 1,
                content: data.data
              }, function(data) { //得到期数信息
                if (data.status == 1) {
            ...
            }else if (source == 'selfs') {
              images[index] = data.data;
              $(".pub-btn").parents('.publish-btn').prev('img').attr('src', data.data);
              upload_vote(data.data);
            } else {
  ...
  //投票图片上传
  function upload_vote(srcs){
    jsonAjax(API.API_LIST.GOLD_VOTE_CHANGEIMG,{aid:aid,userid:getUserid(),image:srcs},function(data){
      if(data.status==1){
        inputTipsText(data.info);
        console.log('upload_vote success!');
      }
    });
  }
  if (i >= num) {
