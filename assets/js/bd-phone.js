    ...
    $.ajax({
        ...
        success: function (data){
              ...
              var Al = 0;
                ...
                $(".ps-pro>ul>li").bind('click',function(){//选择省
                    ...
                    $.cookie('presentstreet','',{expires: -1,path: '/'});
                    ...
                    $(".ps-city>ul>li").bind('click',function(){//选择市
                        $('.ps-city').hide();
...
<<<<<<< .mine
                         $(".ps-dt>ul>li").bind('click',function(){
                            $('.ps-dt').hide();
                            $('.ps-town').slideLeftShow(200);
                            selectedName += " "+$(this).text();
                            districtid = $(this).attr('value');
                            var obj=M();
                            obj.districtid=districtid;
                            obj.districtname=$(this).text();
                            $.cookie('presentdistrictid',JSON.stringify(obj),{expires: 1/144,path: '/'});
                            $.cookie("presentdate", $.now(),{expires: 30,path:'/'});
                            $.cookie('presentdte','',{expires: -1,path: '/'});
                            //pushHistory({title: "deft",url: "#2"});
                            dte = undefined;
                            //console.log($(this).text() + $(this).attr('value'));
                            $('.ps-town .last_select').text(selectedName);
                            $(".ps-town>ul").empty();
                            var hasdistrict = true;
                            for (var i = 0 in citydata) {
                                for(var l=0;l<citydata[i].district.length;l++){
                                    if (citydata[i].district[l].districtid == $(this).attr('value')) {
                                    hasdistrict = citydata[i].district[l].four.length > 0 ? true : false;
                                    for (var k in citydata[i].district[l].four) {
                                        $(".ps-town>ul").append("<li border='bottom' value=" + citydata[i].district[l].four[k].four_id + ">" + citydata[i].district[l].four[k].four_name + "</li>");
                                    }
                                }
                                }
...
=======
                        if(!hasDt){
                            ...
                            $(".ps-dt>ul>li").bind('click',function(){//选择县
>>>>>>> .r86866
                                $('.sel_area').text(selectedName);
                                $.cookie('presentstreet','',{expires: -1,path: '/'});
                                jsonAjax(API.API_LIST.APT4_PUBLIC_GETFOURTHADRESS,{third:this.getAttribute('value')},function(data){
                                    if(data.status==1){
                                        $('.ps-street .last_select').text(selectedName);
                                        var _data=data.data,len=_data.length,html='';
                                        for(var i=0;i<len;i++){
                                            html+="<li border='bottom' value=" + _data[i].id + ">" + _data[i].name + "</li>";
                                        }
                                        $(".ps-street>ul").html(html);
                                        $('.ps-dt').hide();
                                        $('.ps-street').slideLeftShow(200);
                                        $(".ps-street>ul>li").on('click',function(){//选择街道
                                            var _this=$(this);
                                            selectedName+=" "+_this.text();
                                            hidebackground(false);
                                            hideother(false);
                                            $('.ps-street').slideLeftHide(200);
                                            $('.sel_area').text(selectedName);
                                            ...
                                            var obj={
                                                streetid:_this.attr('value'),
                                                streetname:_this.text()
                                            };
                                            $.cookie('presentstreet',JSON.stringify(obj),{expires: 1/144,path: '/'});
                                            $.cookie("presentdate", $.now(),{expires: 30,path:'/'});
                                            selectedName='';
                                            openedSelect = 0;
                                        })
                                ...
                                $(".ps-town>ul>li").bind('click',function(){
                                    $('.ps-town').slideLeftHide(200);
                                    hidebackground(false);
                                    hideother(false);
                                    selectedName += " "+$(this).text();
            ...
            //添加地址成功进行跳转
                ...
                switch (source) {
                    ...
                    case 'vbersky':  //双十一
                        ...
                        break;
                    case 'mall':  //汽车服务积分商品--确认订单
                        obj.source = source;
                        obj.goods_id = goods_id;
                        obj.product_id =I('product_id','');
                        obj.num = I("num");
                        Interface('gotoMallDeterOrder',obj);
                        break;
                    case 'omall':  //汽车服务普通商品--确认订单
                        obj.source = source;
                        obj.goods_id = goods_id;
                        obj.product_id =I('product_id','');
                        obj.num = I("num");
                        Interface('gotoDeterOrder',obj);
                        break;
                    case 'lotteryOrder':  //跳转到确认订单页面
                        obj.useraddress_id = data.data.id;
                        obj.goods_id = goods_id;
                        obj.is_crowd = is_crowd;
                        obj.source='lotteryOrder';
                        Interface('gotoDeterOrder', obj);
                        break;
                    default:
                        //obj.pirzelog_id = I('pirzelog_id', 0);
...
function alter(name, telphone, proid, cityid, dte, detadd) {
    showLoad('正在请求');
        ...
        if (data.status == 1) {
            ...
            if(source=='order'){
            ...
            }if(source=='lotteryOrder'){
                var param = M();
                param.source = source;
                param.goods_id = goods_id;
                param.product_id = product_id;
                param.useraddress_id = data.data.id;
                param.num = I("num");
                var _obj=M();
                _obj.useraddress_id = ide;
                _obj.source = source;
                ...
                obj.brand_id=brand_id;
                console.log(obj);
                changeBackUrlL('vbersky/confirmorder',obj);
            }else if(source=="mall"){
                var param = M();
                param.source = source;
                param.goods_id = goods_id;
            ...
            Interface('gotoAddressMsg', param);  //地址管理列表
        } else {
            inputTipsText(data.info);
        }
    });
