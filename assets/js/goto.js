...
function gotoKeepIndex(param){
    redirect('/web/carsvc-landrover/keep-car/after-sale.html', param);
}
/*专属公众号收银员*/
...
function gotoT(param) {   //交易大厅
    param.isshare=1;
    redirect('/web/lottery/notice.html',param);//暂跳转到系统维护通知
...
function gotoReceiveImmediately(param) { //抽奖专区--领奖界面
    redirect('/web/interact/prize/accept.html',param);
}
//神马大楼
...
//喵喵在途推荐文章跳转
function gotoCarnivalArtical1(param) {
    redirect('/web/smdg/carnival-artical1.html', param);
};
//喵喵在途推荐社区跳转
function gotoCarnivalArtical2(param) {
    redirect('/web/smdg/carnival-artical2.html', param);
};
function gotoCommentReply(param) {
    redirect('/web/smdg/comment-reply.html', param);
};
//喵喵在途的评论回复列表
function gotoCommentReply1(param) {
    redirect('/web/smdg/comment-reply1.html', param);
};
//查看个人详情
...
//车队介绍
function gotoFleetIntroduce(param){
    redirect('/web/smdg/fleet-introduce.html',param);
}
// 编辑车队介绍
function gotoFleetIntroduceEdit(param){
    redirect('/web/smdg/fleet-introduceEdit.html',param);
}
...
function gotoCardWish(param){ // 许愿树首页
    redirect('/web/interact/card/wish/index.html',param);
}
function gotoCardWishResult(param){ // 许愿树支付结果（开奖页）
    redirect('/web/interact/card/wish/pay-result.html',param);
}

function gotoRewardList(param) {//奖励卡
    redirect('/web/interact/card/reward-list.html',param);
}
//神马卡牌相关跳转 - over
...
function gotoAwardCashier(param){//嘉年华赞赏页
    redirect('/web/lottery/award-cashier.html',param);
}
function gotoAwardRecord(param){//嘉年华赞赏页
    redirect('/web/smdg/award-record.html',param);
}
function gotoCarnivalComment(param){//嘉年华赛事评论
    redirect('/web/smdg/carnival-comment.html',param);
}
function gotoCommentAllReply(param){//嘉年华赛事回复页面
    redirect('/web/smdg/comment-all-reply.html',param);
}
function gotoScore(param) {
    redirect('/web/smdg/power-list.html',param);
}
/**天荣汽配城**/
function gotoProductIntroduction(param){//产品介绍页面
    redirect('/web/tianrongqipei/product-introduction.html',param);
}
function gotoTianRongIndex(param){//主页页面
    redirect('/web/tianrongqipei/index.html',param);
}
function gotoAboutUs(param){//关于我们页面
    redirect('/web/tianrongqipei/aboutus.html',param);
}
function gotoContactUs(param){//关于联系我们页面
    redirect('/web/tianrongqipei/contactus.html',param);
}
function gotoJoinUs(param){//关于入驻页面
    redirect('/web/tianrongqipei/settledin.html',param);
}
function downshenmahuiApp(param) {//神马汇app下载页
    param.isshare = 1;
    redirect('http://www.didi365.com/Home/Qr/smh', param);
}
function downmiaomiaoApp(param) {//喵喵在途app下载页
    param.isshare = 1;
    redirect('https://www.didi365.com/home/qr/mmzt', param);
}
function toShenMaHuiGoodsDetail(param) {//神马汇商品详情页
    redirect('/web/shenmahui/goods-detail.html', param);
}
function gotoCarLandMyInfo(param){//智慧车间--会员中心
    param.car_public=1;
    redirect('/web/carsvc-landrover/myinfo.html',param);
}
function gotoCarnivalRules(param){//嘉年华投票规则
    redirect('/web/smdg/carnival-rules.html',param);
}
function gotoMallIndex(param){//智慧车间--网上商城首页
    redirect('/web/carsvc-landrover/mall/index.html',param);
}
function gotoMyorder(param){//智慧车间--我的订单付款完成后跳转
    redirect('/web/carsvc-landrover/personal-order.html',param);
}
function gotoMBKBindex(param){//萌宝酷爸活动首页
    redirect('/web/mengbaokuba/index.html',param);
}
function gotoMBKBpersonal(param){//萌宝酷爸活动个人主页
    redirect('/web/mengbaokuba/personal.html',param);
}
function gotoMBKBsubmit(param){//萌宝酷爸活动个人注册
    redirect('/web/mengbaokuba/submit-enroll-info.html',param);
}
